import { DatabaseConnection, Database, TableInfo, TableColumn, TableIndex, TableAdd } from '@/types/db_manage';
...
export const fetchTableStructure = async (tableId: number): Promise<TableColumn[]> => {
  ...
  return axiosInstance.get<PageData<TableColumn>>('/field/fields', params )
    .then((res) => {
...
export const addField = async (connectionId: string, databaseId: string, tableId: string, field: TableColumn): Promise<void> => {
  // Simulated API call
