...
import MenuModifyComponent from "@/views/system/menu/components/menu-modify";
import MenuQueryComponent from "@/views/system/menu/components/menu-query";
...
const Menu: React.FC = () => {
  ...
  // 详情模块
  const [isMenuDetailDrawerVisible, setIsMenuDetailDrawerVisible] = useState<boolean>(false);
  ...
  const [visibleColumns, setVisibleColumns] = useState(menuPageColumns.map(col => col.key));
  ...
  const filteredMenuColumns = menuPageColumns.filter(col => visibleColumns.includes(col.key));
  ...
  // 新增模块
  const [isMenuCreateModalVisible, setIsMenuCreateModalVisible] = useState(false);
  ...
  const handleSelectionChange = (selectedRowKeys: React.Key[], selectedRows: MenuPage[]) => {
  ...
  // 单个更新模块
  const [isMenuModifyModalVisible, setIsMenuModifyModalVisible] = useState<boolean>(false);
  const [isMenuModifyLoading, setIsMenuModifyLoading] = useState<boolean>(false);
  ...
  const handleMenuModifyFinish = async () => {
    const menuModifyData = (await menuModifyForm.validateFields()) as MenuModify;
  ...
  const [isMenuBatchModifyModalVisible, setIsMenuBatchModifyModalVisible] = useState<boolean>(false);
  const [isMenuBatchModifyLoading, setIsMenuBatchModifyLoading] = useState<boolean>(false);
  ...
  const handleMenuBatchModifyFinish = async () => {
    const menuBatchModify = (await menuBatchModifyForm.validateFields()) as MenuBatchModify;
  ...
  // 导入模块
  const [isMenuImportModalVisible, setIsMenuImportModalVisible] = useState<boolean>(false);
  const [isMenuImportLoading, setIsMenuImportLoading] = useState<boolean>(false);
